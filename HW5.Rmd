...
for( i in 1:100) {
  ...
  beta.ols = coef(nk.ols)
  X.ols = model.matrix(Y ~ . -mu, data=df)
  RMSE.beta[i] = sqrt(mean((betatrue - beta.ols)^2))
  RMSE.mu[i] = sqrt(mean((mu - X.ols %*% beta.ols)^2))
...
load(fname[1])
aa = lm(Y ~ . -mu, data=df[1:50,])
aa_2 = step(aa, k = 2, trace = 0)
summary(aa)
summary(aa_2)
```
