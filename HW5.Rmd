...
$$ Var[x_i\beta | Y, g, \phi] = x_i Var[\beta | Y, g, \phi] x_i^t$$

$$ = x_i \dfrac{g}{g + 1} \sigma^2(X^TX)^{-1} x_i^t $$

*In the derivations below, the $\epsilon$, $X$, represent those associated with the test set. While we could have taken pains to label each of them "test", we have left this notation out for the sake of simplicity.

c) $$ E[Y^{test} | Y, g, \phi] = E[ 1_n \beta_{0} + X\beta + \epsilon^ | Y, g, \phi]$$

$$ = E[ 1_n \beta_{0} | Y, g, \phi] + E[X\beta | Y, g, \phi] + E[\epsilon^ | Y, g, \phi] $$

$$ = \bar{Y} + x_i \dfrac{g}{1 + g} (X^TX)^{-1}X^Ty + 0$$

$$ = \bar{Y} + x_i \dfrac{g}{1 + g} (X^TX)^{-1}X^Ty $$

$$Var[Y^{test} | Y, g, \phi] =  Var[ 1_n \beta_{0} + X\beta + \epsilon^ | Y, g, \phi] $$

$$ = Var[1_n \beta_{0} | Y, g, \phi] + Var[X \beta | Y, g, \phi] + Var[\epsilon^ | Y, g, \phi] $$

$$ = \dfrac{1}{n \phi} + x_i\dfrac{g}{g + 1}\sigma^2(X^TX)^{-1}x_i^t + \dfrac{I_n}{\phi}$$

$$ = \dfrac{1}{\phi} ( \dfrac{1}{n} + x_i\dfrac{g}{g + 1}(X^TX)^{-1}x_i^t + I_n) $$
