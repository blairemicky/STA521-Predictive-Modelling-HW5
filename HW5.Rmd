...
The number of "correct" models is `count_true`. The average RMSE for $\beta^{true}$, $\mu^{true}$ and out of sample prediction are `r avg_rmse_aic[1]`, `r avg_rmse_aic[2]`, and `r avg_rmse_aic[3]`, respectively. The full distributions are found in the plots above.
...
$$ a= \dfrac{1}{\phi} \left( I_n\left(1+\dfrac{1}{n}\right) + X^\ast\dfrac{g}{g + 1}(X^TX)^{-1}X^{\ast t}  \right) $$
...
## Appendix: Doing the full calculations for problems 7, 8 and 9
...
ANSWER: a) By Bayes' theorem we have
...
Note: In the derivations below, we replced "test" by a $*$ for the sake of simplicity.
...
ANSWER: First we take the shortcut, and then we do the actual calculations.

SHORTCUT VERSION:
a) From the theorem for the marginal distribution of a Normal-Gamma, we know that if
...
$$ = \int (2\pi)^{-p/2} \begin{vmatrix}x_i\dfrac{g}{(g + 1)\phi}(X^TX)^{-1}x_i^t\end{vmatrix}^{-1/2}  exp(-\dfrac{(\beta - x_i\dfrac{g}{g + 1}\hat{\beta}_{OLS})^T (x_i\dfrac{g}{g + 1}\sigma^2(X^TX)^{-1}x_i^t)^{-1} (\beta - x_i\dfrac{g}{g + 1}\hat{\beta}_{OLS})}{2}) \dfrac{(\dfrac{\nu_{n}\hat{\sigma^2}}{2})}{\Gamma(\nu_{n}/2)}^{\nu_{n}/2} \phi^{\nu_{n}/2 - 1} exp(-\dfrac{\nu_{n}\hat{\sigma^2}}{2}) d\phi $$

$$ \propto \int \phi^{p/2} \phi^{\nu_{n}/2 -1} exp(-\dfrac{(\beta - x_i\dfrac{g}{g + 1}\hat{\beta}_{OLS})^T (x_i^{-1}\dfrac{g + 1}{g }\phi(X^TX)(x_i^t)^{-1}) (\beta - x_i \dfrac{g}{g + 1}\hat{\beta}_{OLS})}{2}\phi) exp(-\dfrac{\nu_{n}\hat{\sigma^2}}{2}) d\phi $$
...
$$ = \int (2\pi)^{-p/2} \begin{vmatrix}\dfrac{1}{\phi} ( \dfrac{1}{n} + x_i\dfrac{g}{g + 1}(X^TX)^{-1}x_i^t + I_n)\end{vmatrix}^{-1/2}  exp(-\dfrac{(Y^{test} - \bar{Y} + x_i \dfrac{g}{1 + g}\hat{\beta}_{OLS})^T (\dfrac{1}{\phi} ( \dfrac{1}{n} + x_i\dfrac{g}{g + 1}(X^TX)^{-1}x_i^t + I_n))^{-1} (Y^{test} - \bar{Y} + x_i \dfrac{g}{1 + g}\hat{\beta}_{OLS})}{2}) \dfrac{(\dfrac{\nu_{n}\hat{\sigma^2}}{2})}{\Gamma(\nu_{n}/2)}^{\nu_{n}/2} \phi^{\nu_{n}/2 - 1} exp(-\dfrac{\nu_{n}\hat{\sigma^2}}{2}) d\phi $$

$$ \propto \int \phi^{p/2} \phi^{\nu_{n}/2 -1} exp(-\dfrac{(Y^{test} - \bar{Y} + x_i \dfrac{g}{1 + g}\hat{\beta}_{OLS})^T  ( \dfrac{1}{n} + x_i\dfrac{g}{g + 1}(X^TX)^{-1}x_i^t + I_n)^{-1} (Y^{test} - \bar{Y} + x_i \dfrac{g}{1 + g}\hat{\beta}_{OLS})}{2}\phi) exp(-\dfrac{\nu_{n}\hat{\sigma^2}}{2}) d\phi $$

$$ \propto \int \phi^{(p+\nu_{n})/2 -1} exp(-\dfrac{(Y^{test} - \bar{Y} + x_i \dfrac{g}{1 + g}\hat{\beta}_{OLS})^T  ( \dfrac{1}{n} + x_i\dfrac{g}{g + 1}(X^TX)^{-1}x_i^t + I_n)^{-1} (Y^{test} - \bar{Y} + x_i \dfrac{g}{1 + g}\hat{\beta}_{OLS}) + \nu_{n}\hat{\sigma^2} }{2}\phi) d\phi $$

As in the previous two derivations, we know this is equal to the reciprocal of the proportional constant of the gamma:

$$  = \Gamma(\dfrac{p + \nu_{n}}{2}) \Big[\dfrac{(Y^{test} - \bar{Y} + x_i \dfrac{g}{1 + g}\hat{\beta}_{OLS})^T  ( \dfrac{1}{n} + x_i\dfrac{g}{g + 1}(X^TX)^{-1}x_i^t + I_n)^{-1} (Y^{test} - \bar{Y} + x_i \dfrac{g}{1 + g}\hat{\beta}_{OLS}) + \nu_{n}\hat{\sigma^2} }{2}\Big]^{-(p + \nu_{n})/2} $$
