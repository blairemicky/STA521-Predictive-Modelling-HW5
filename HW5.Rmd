...
b2<-ggplot(data = RMSE[2], aes(x = RMSE[2])) + 
  geom_histogram(fill = "khaki", color = "darkgoldenrod")+
  geom_vline(xintercept = mean(RMSE[,2]), color = "darkblue")+ labs(title="BMA: RMSE for beta ", x="RMSE", y="Count")
#test
b3<-ggplot(data = RMSE[3], aes(x = RMSE[3])) + 
  geom_histogram(fill = "salmon", color = "orangered3")+
  geom_vline(xintercept = mean(RMSE[,3]), color = "darkblue")+ labs(title="BMA: RMSE for beta ", x="RMSE", y="Count")
...
print(confint.coef.bas(coef(app_best_BMA12)))
...
#beta
ggplot(data = RMSE[1], aes(x = RMSE[1])) + 
  geom_histogram(fill = "lightblue", color = "darkblue")+
  geom_vline(xintercept = mean(RMSE[,1]), color = "red")+ labs(title="BPM: RMSE for beta ", x="RMSE", y="Count")
#mu
ggplot(data = RMSE[2], aes(x = RMSE[2])) + 
  geom_histogram(fill = "khaki", color = "darkgoldenrod")+
  geom_vline(xintercept = mean(RMSE[,2]), color = "darkblue")+ labs(title="BPM: RMSE for beta ", x="RMSE", y="Count")
#test
ggplot(data = RMSE[3], aes(x = RMSE[3])) + 
  geom_histogram(fill = "salmon", color = "orangered3")+
  geom_vline(xintercept = mean(RMSE[,3]), color = "darkblue")+ labs(title="BPM: RMSE for beta ", x="RMSE", y="Count")
count_true
...
for(i in 1:100){
  ...
  pred.df = predict(df.bas[[i]], newdata=df[51:75,], estimator="HPM")
  ...
  ## for mu
  RMSE_mu[i] = rmse(mu[1:50], muhat) 
  ## for testing data
  RMSE_test[i] = rmse(Y[51:75],predvalues)    
}
...
for(i in 1:100){
  ...
  df.bas[[i]] = bas.lm(Y ~.-mu, data=df[1:50,],
                    ...
                    initprobs="marg-eplogp")
  #get the BMA model
  betas.bas = coef(df.bas[[i]])
  betahat=betas.bas$postmean
   
  #the fitted values
  muhat = fitted(df.bas[[i]], estimator="BMA")
  #the predicted values
  pred.df = predict(df.bas[[i]], newdata=df[51:75,], estimator="BMA")
  predvalues = pred.df$fit
...
for(i in 1:100){
  ...
  #the fitted values
  muhat = fitted(df.bas[[i]], estimator="BPM")
  #the predicted values
  pred.df = predict(df.bas[[i]], newdata=df[51:75,], estimator="BPM")
  predvalues = pred.df$fit
