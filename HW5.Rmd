...
for (i in 1:100) {
  ...
  X[1:50,] = sweep(X[1:50,], 2, apply(X[1:50,],2, mean)) 
  X[51:75,] = sweep(X[51:75,], 2, apply(X[51:75,],2, mean)) 
...
```{r, Prob2, echo=FALSE, message=FALSE, warning=FALSE}
...
for( i in 1:100) {
  ...
  RMSE.mu[i] = sqrt(mean((df$mu[1:50] - X.ols %*% beta.ols)^2))
  RMSE.test.pred[i] = sqrt(mean((df$Y[51:75] - predict(ls[[i]], newdata = df[51:75,]))^2))
}
...
#histogram: beta
p1<-ggplot(data = rmse.df[1], aes(x = rmse.df[1])) + theme_light() + labs(x = "RSME: Beta", y = "Count") + geom_histogram(fill = "lightblue", color = "darkblue",bins=30) + geom_vline(xintercept = avg.rmse.beta, color = "red")+labs(title="OLS Model")
...
```{r, Prob3, echo=FALSE, cache=TRUE, warning=FALSE, message=FALSE}
...
for(i in 1:100){
  ...
  ## for mu
  RMSE_mu_aic[i] = rmse(df$mu[1:50], app_best$fitted.values)
  ## for testing data
  RMSE_test_aic[i] = rmse(df$Y[51:75],predict(app_best, newdata = df[51:75,]))
}
...
avg_rmse_aic<-data.frame(beta = avg_rmse_beta, mu = avg_rmse_mu, test = avg_rmse_test )

#store RSME's in data frame
rmse_df_aic = data.frame(RMSE_beta_aic, RMSE_mu_aic, RMSE_test_aic)
#beta
p1<-ggplot(data = rmse_df_aic[1], aes(x = rmse_df_aic[1])) + 
  geom_histogram(fill = "lightblue", color = "darkblue")+ labs(title="AIC Step Selection", x = "RSME: Beta", y = "Count") +
...
# number of times true model selected
print(count_true)
propto.aic = count_true/100

```

The number of "correct" models is `count_true` which is equal to 0. The average RMSE for $\beta^{true}$, $\mu^{true}$ and out of sample prediction are `r avg_rmse_aic[1]`, `r avg_rmse_aic[2]`, and `r avg_rmse_aic[3]`, respectively. The full distributions are found in the plots above.
...
From the figure we see that with the exception of coefficients  13, 18, 19 and 20 the confidence intervals include the true value of $\beta$. Coefficients 13, 18 and 19 are not included in the model (set to zero) although their true values are non-zero. Conversely, coefficeint 8 is included and non-zero although the true value is zero.
...
```{r, Prob5, echo=FALSE, cache=TRUE, message=FALSE, warning=FALSE}
#create vector with the same length and same name of betatrue
beta_like = numeric(length(betatrue))
names(beta_like) = names(betatrue)
...
for(i in 1:100){
  ...
  ## for mu
  RMSE_mu_bic[i] = rmse(df$mu[1:50], app_best$fitted.values)
  ## for testing data
  RMSE_test_bic[i] = rmse(df$Y[51:75],predict(app_best, newdata = df[51:75,]))
}
...
avg_rmse_bic<-data.frame(beta = avg_rmse_beta,mu = avg_rmse_mu,test = avg_rmse_test)
...
p1<-ggplot(data = rmse_df_bic[1], aes(x = rmse_df_bic[1])) + 
  ...
  geom_vline(xintercept = avg_rmse_beta, color = "red")+ labs(title="BIC Step Selection", x = "RSME: Beta", y = "Count")
#mu
p2<-ggplot(data = rmse_df_bic[2], aes(x = rmse_df_bic[2])) + 
  geom_histogram(fill = "khaki", color = "darkgoldenrod")+
  geom_vline(xintercept = avg_rmse_mu, color = "darkblue")+ labs( x = "RSME: Mu", y = "Count")
#test
p3<-ggplot(data = rmse_df_bic[3], aes(x = rmse_df_bic[3])) + 
  geom_histogram(fill = "salmon", color = "orangered3")+
  geom_vline(xintercept = avg_rmse_test, color = "darkblue")+ labs(x = "RSME: Test prediction", y = "Count")

multiplot(p1,p2,p3, cols = 3)

# count of number of times true is selected
print(count_true)

propto.bic = count_true/100

```
The number of "correct" models is `count_true` which is also equal to 0. The average RMSE for $\beta^{true}$, $\mu^{true}$ and out of sample prediction are `r avg_rmse_bic[1]`, `r avg_rmse_bic[2]`, and `r avg_rmse_bic[3]`, respectively.
...
From the figure we see that with the exception of coefficients 13, 18 and 19, and 20, the confidence intervals include the true value of $\beta$. Coefficeints 13, 18 and 19 are not included in the best model although their true values are non-zero.
...
ANSWER: 
While we provide short answers here to #7, #8, and #9, full derivations of each problem are shown at the bottom of the document in the Appendix.

a) For a short answer, we reference the class notes which provide the posterior  $$ \beta | Y, \phi, g \sim \mathcal{N}\left(\frac{g}{1+g}\hat{\beta},  \frac{g}{1+g}(X^tX)^{-1}\phi^{-1}\right),$$ and $$\beta_0 \, | \, Y, \phi \sim \mathcal{N}(\bar{Y}, I_n/n\phi).$$

b) We note that technically $\mu_i = \beta_0 + x_i^t \beta$, but here we are asked to consider only the second term, $$\mu_i = x_{i}^t\beta | Y, g, \phi. $$ This is an affine transformation of a Gaussian, and hence by standard results (see, e.g.,  the book by Christensen on linear models), we know that $\mu_i$ is normally distributed as : $$ \mu_{i}\sim \mathcal{N}\left( x_i \dfrac{g}{1+g}\hat{\beta}, \phi^{-1}\dfrac{g}{1+g}x_i (X^TX)^{-1} x_i^t  \right).$$

Note: In the derivations below, we replaced "test" by a $*$.

c) Because  $$ Y^\ast | Y, g, \phi =  (1_n \beta_{0} + X^\ast \beta + \epsilon^\ast )| Y, g, \phi $$ is the sum of three Gaussians, it suffices to calculate the mean and variance. 

$$ E[Y^\ast | Y, g, \phi] = E[ 1_n \beta_{0} + X^\ast \beta + \epsilon^\ast | Y, g, \phi]$$

$$ = E[ 1_n \beta_{0} | Y, g, \phi] + E[X^\ast\beta | Y, g, \phi] + E[\epsilon^\ast | Y, g, \phi] $$

$$ = \bar{Y} + x_i \dfrac{g}{1 + g} \hat{\beta} + 0$$

$$ = \bar{Y} + x_i \dfrac{g}{1 + g}\hat{\beta} $$

For the calculation of the variance, we note that the $\beta_0$ and $\beta$ are independent random variables conditioned on $Y$ and $\phi$, and $\epsilon^\ast$ is independent of any past observation and hence independent of the model coefficients. Therefore, we can write
$$Var[Y^\ast | Y, g, \phi] =  Var[ 1_n \beta_{0} + X^\ast \beta + \epsilon^\ast  | Y, g, \phi] $$

$$ = Var[1_n \beta_{0} | Y, g, \phi] + Var[X^\ast \beta | Y, g, \phi] + Var[\epsilon^\ast | Y, g, \phi] $$

$$ = \dfrac{I_n}{n \phi} + X^\ast\dfrac{g}{1+g}\phi^{-1}(X^TX)^{-1}X^{\ast t} + \dfrac{I_n}{\phi}$$

$$ = \dfrac{1}{\phi} \left( I_n\left(1+\dfrac{1}{n}\right) + X^\ast\dfrac{g}{g + 1}(X^TX)^{-1}X^{\ast t}  \right) $$
...
ANSWER: No, $\beta$ and $\beta_0$ are no longer independent. In fact, they have a joint dependency on $\phi$ which is now integrated out and entangles the two.


First we take the shortcut, and then we do the actual calculations. 

a) From the theorem for the marginal distribution of a Normal-Gamma, we know that if $\theta\sim N(m, 1/\phi \Sigma)$ and $\phi\sim G(\nu/2, \nu \hat{\sigma}^2/2)$ then $\theta \sim t_\nu (m, \hat{\sigma}^2 \Sigma)$. Here we have (see class notes)  $\beta\, |\, Y, \phi \sim N(m, 1/\phi \Sigma)$ for $m=\frac{g}{1+g} \hat{\beta}$ and $\Sigma=\frac{g}{1+g} (X^tX)^{-1}$,  and $\phi \, | \, Y \sim G(\nu/2, \nu \hat{\sigma}^2/2)$ with $\nu=n-1$ and $$
\hat{\sigma}^2=(n-1)\left(SSE-\frac{1}{1+g} \hat{\beta}^t (X^tX)^{-1}\hat{\beta}\right).$$ It follows from the above theorem that $\beta$ is a multivariate Student t distribution
$$ \beta\,  |\,  Y \sim t_{n-1} \left(\frac{g}{1+g} \hat{\beta}, \frac{g(n-1)}{1+g}\left(SSE-\frac{1}{1+g} \hat{\beta}^t (X^tX)^{-1}\hat{\beta}\right) (X^tX)^{-1}\right). $$

Similarly, we find that for $\beta_0 \, | \, Y, \phi \sim \mathcal{N} (\bar{Y} ,\frac{1}{\phi}( \frac{I_n}{n}))$
we obtain 
$$ \beta_0 \,  |\,  Y \sim t_{n-1} \left(\bar{Y}, \frac{n-1}{n}\left(SSE-\frac{1}{1+g} \hat{\beta}^t (X^tX)^{-1}\hat{\beta}\right) I_n \right). $$
...
$$ \mu_i\,  |\,  Y \sim t_{n-1} \left(\frac{g}{1+g} x_i^t\hat{\beta}, \frac{g(n-1)}{1+g}\left(SSE-\frac{1}{1+g} \hat{\beta}^t (X^tX)^{-1}\hat{\beta}\right)x_i^t  (X^tX)^{-1} x_i\right).$$


c) From 7c) we know that  $Y^\ast | Y, \phi =  (1_n \beta_{0} + X^\ast \beta + \epsilon^\ast )| Y$ has distribution
$$Y^\ast|Y, \phi \sim \mathcal{N}\left(\bar{Y} + x_i \dfrac{g}{1 + g}\hat{\beta} ,  \dfrac{1}{\phi} \left( I_n\left(1+\dfrac{1}{n}\right) + X^\ast\dfrac{g}{g + 1}(X^TX)^{-1}X^{\ast t}  \right) \right).$$ 
Furthermore, $\phi \, | \, Y \sim G(\nu/2, \nu \hat{\sigma}^2/2)$ with $\nu=n-1$ and $$
\hat{\sigma}^2=(n-1)\left(SSE-\frac{1}{1+g} \hat{\beta}^t (X^tX)^{-1}\hat{\beta}\right).$$ It follows from the above theorem that $Y*|Y$ has a multivariate Student t distribution with
$$ Y^\ast|Y \sim t_{n-1}\left(\bar{Y} + x_i \dfrac{g}{1 + g}\hat{\beta},  (n-1) \left( I_n\left(1+\dfrac{1}{n}\right) + X^\ast\dfrac{g}{g + 1}(X^TX)^{-1}X^{\ast t}\right)\left(SSE-\frac{1}{1+g} \hat{\beta}^t (X^tX)^{-1}\hat{\beta}\right)\right) $$
...
ANSWER:
Again we use the theorem on the marginal of a normal-gamma. Here, we have that $\beta \, | \, \beta_0, \phi,  \tau \sim \mathcal{N} (m, \frac{1}{\tau} \Sigma)$ with $m=0$ and $\Sigma=(X^tX \phi)^{-1}$, and $\tau \sim G(\nu/2, \nu \hat{\sigma}^2/2)$ with $\nu=1$ and $\hat{\sigma}^2=n$. It follows that the distribution 
$$ \beta\, | \, \beta_0, \phi \sim t_1(0, n (X^tX \phi)^{-1}).$$
...
```{r, Prob 10, echo=FALSE, cache=TRUE, warning=FALSE, message=FALSE}
coef.HPM = function(object) {
  ...
  post.mean[1] = post.mean[1]/object$shrinkage[best]   # leave intercept alone 
...
coef.BPM = function(object) {
  ...
  post.mean[1] = post.mean[1]/object$shrinkage[yy$best]
  return(list(HPM = model, betahat = post.mean))
}
...
for(i in 1:100){
  ...
  # create beta_bpm of length(betatrue) with zeros for left out predictors
...
#store RSME's in data frame
RMSE_HPM = data.frame(beta = RMSE_beta_HPM, mu = RMSE_mu_HPM, test = RMSE_test_HPM)
avg.rmse.hpm= apply(RMSE_HPM, 2, mean)

RMSE_BMA = data.frame(beta = RMSE_beta_BMA, mu = RMSE_mu_BMA, test = RMSE_test_BMA)
avg.rmse.bma = apply(RMSE_BMA, 2, mean)

RMSE_BPM = data.frame(beta = RMSE_beta_BPM, mu = RMSE_mu_BPM, test = RMSE_test_BPM)
avg.rmse.bpm = apply(RMSE_BPM, 2, mean)


propto.hpm = count_true_hpm/100
propto.bpm = count_true_bpm/100
```



```{r, 10_output, echo=FALSE, cache=TRUE, message=FALSE, warning=FALSE}
RMSE<-RMSE_HPM

#beta
g1<-ggplot(data = RMSE[1], aes(x = RMSE[1])) + 
  geom_histogram(fill = "lightblue", color = "darkblue")+
  geom_vline(xintercept = mean(RMSE[,1]), color = "red") + labs(title="RMSEs for HPM", x="RMSE for beta", y="Count")
#mu
g2<-ggplot(data = RMSE[2], aes(x = RMSE[2])) + 
  geom_histogram(fill = "khaki", color = "darkgoldenrod")+
  geom_vline(xintercept = mean(RMSE[,2]), color = "darkblue")+ labs( x="RMSE for mu", y="Count")
#test
g3<-ggplot(data = RMSE[3], aes(x = RMSE[3])) + 
  geom_histogram(fill = "salmon", color = "orangered3")+
  geom_vline(xintercept = mean(RMSE[,3]), color = "darkblue")+ labs(x="RMSE for prediction", y="Count")
...
The number of times the "correct" model is selected is `r count_true_hpm`. The mean RMSEs for $\beta^{true}$, $\mu^{true}$ and $Y^{pred}$ were `r mean(RMSE_beta_HPM)`, `r mean(RMSE_mu_HPM)` and `r mean(RMSE_test_HPM)`, respectively. Since we used MCMC, we checked the 'pip' plots for several datasets. We found them to be satisfatory, see below for the diagnostics of the last data set.
...
app_best = bas.lm(Y~.-mu, data=df[1:50,],
                  ...
                  method="MCMC", MCMC.iterations = 200000, thin = 20)
mod_hpm=rep(0,21)
mod_hpm[coef.HPM(app_best)$HPM+1]<-1
best_mod =     bas.lm(Y~.-mu, data=df[1:50,],a=nrow(df[1:50,]), prior="g-prior",n.models=1,bestmodel=mod_hpm )

MCT<-as.data.frame(cbind(confint(coef(best_mod)),betatrue) )

colnames(MCT)<-c( "lci", "uci","mean","truth")
ggplot(MCT, aes(x=names(betatrue), y=mean),show.legend = TRUE) + 
  geom_errorbar(aes(ymin=lci, ymax=uci), width=.1) +
  geom_point()+geom_point(aes(y=truth),color="red")+labs(title="Posterior Confidence Intervals for HPM (black=estimate, red=truth)", x="Coefficients", y="Value")
```
...
```{r, 13_output, echo=FALSE, cache=TRUE, warning=FALSE, message=FALSE}
RMSE<-RMSE_BPM
...
app_best = bas.lm(Y~.-mu, data=df[1:50,],
                  ...
                  method="MCMC", MCMC.iterations = 200000, thin = 20)
mod_bpm=rep(0,21)
mod_bpm[coef.BPM(app_best)$HPM+1]<-1
best_mod =     bas.lm(Y~.-mu, data=df[1:50,],a=nrow(df[1:50,]), prior="g-prior",n.models=1,bestmodel=mod_bpm )
...
ggplot(MCT, aes(x=names(betatrue), y=mean),show.legend = TRUE) + 
  geom_errorbar(aes(ymin=lci, ymax=uci), width=.1) +
  geom_point()+geom_point(aes(y=truth),color="red")+labs(title="Posterior Confidence Intervals for BPM (black=estimate, red=truth)", x="Coefficients", y="Value")
...
```{r, Prob14.1, echo=FALSE, cache=TRUE, warning=FALSE, message=FALSE}
...
#store RSME's in data frame
RMSE_HPM = data.frame(beta = RMSE_beta_HPM, mu = RMSE_mu_HPM, test = RMSE_test_HPM)
avg.rmse.zs.hpm= apply(RMSE_HPM, 2, mean)

RMSE_BMA = data.frame(beta = RMSE_beta_BMA, mu = RMSE_mu_BMA, test = RMSE_test_BMA)
avg.rmse.zs.bma = apply(RMSE_BMA, 2, mean)

RMSE_BPM = data.frame(beta = RMSE_beta_BPM, mu = RMSE_mu_BPM, test = RMSE_test_BPM)
avg.rmse.zs.bpm = apply(RMSE_BPM, 2, mean)

propto.zs.hpm = count_true_hpm/100
propto.zs.bpm = count_true_bpm/100
```

```{r, Prob14.2, echo=FALSE, cache=FALSE}
### OUTPUT FOR 10
...
mod_hpm=rep(0,21)
mod_hpm[coef.HPM(app_best)$HPM+1]<-1
best_mod =     bas.lm(Y~.-mu, data=df[1:50,],a=nrow(df[1:50,]), prior="ZS-null",n.models=1,bestmodel=mod_hpm )
...
```{r, Prob14.4, echo=FALSE, cache=FALSE}
# For Question 12
RMSE<-RMSE_BMA
...
```{r, Prob14.5, echo=FALSE, cache=FALSE}
...
```{r, echo=FALSE}
#Create columns for each set of beta, mu, and test RMSE
methods = rbind(OLS = avg.rmse.df, AIC = avg_rmse_aic, BIC = avg_rmse_bic, HPM.ZG = avg.rmse.hpm, BMA.ZG = avg.rmse.bma, BPM.ZG = avg.rmse.bpm, HPM.ZS = avg.rmse.zs.hpm, BMA.ZS = avg.rmse.zs.bma, BPM.ZS = avg.rmse.zs.bpm)

#Create table for average RMSE's
kable(methods, row.names = TRUE, caption = "Average RMSE's by Method")

#Create columns for proportion true
proportions = rbind(AIC = propto.aic, BIC = propto.bic, HPM = propto.hpm, BPM = propto.bpm, ZS.HPM = propto.hpm, ZS.BPM = propto.bpm)

#Create table for proportion true across methods
kable(proportions, col.names = "Proportion", row.names = TRUE, caption = "Proportion of Time True Model Selected")
```
...
$$E[\beta | Y, g, \phi] = \dfrac{g}{g + 1}(X^TX)^{-1}X^T Y= \frac{g}{1+g}\hat{\beta} $$

In summary, we find that $$ \beta | Y, \phi, g \sim \mathcal{N}\left(\frac{g}{1+g}\hat{\beta},  \frac{g}{1+g}(X^tX)^{-1}\phi^{-1}\right)$$


b) We note that $$ \mu_{i} = x_{i}^t\beta | Y, g, \phi $$ is an affine transformation of a Gaussian, and hence by standard results (see, e.g.,  the book by Christensen on linear models), we know that $\mu_i$ is normally distributed as : $$ \mu_{i}\sim \mathcal{N}\left( x_i \dfrac{g}{1+g}\hat{\beta}, \phi^{-1}\dfrac{g}{1+g}x_i (X^TX)^{-1} x_i^t  \right).$$


c) $$ E[Y^\ast | Y, g, \phi] = E[ 1_n \beta_{0} + X^\ast \beta + \epsilon^\ast | Y, g, \phi]$$
...
$$ = \dfrac{I_n}{n \phi} + x_i\dfrac{g}{g + 1}\phi^{-1}(X^TX)^{-1}x_i^t + \dfrac{I_n}{\phi}$$

$$ = \dfrac{1}{\phi} \left( I_n\left(1+\dfrac{1}{n}\right) + x_i\dfrac{g}{g + 1}(X^TX)^{-1}x_i^t  \right) $$
...
$$ = \int (2\pi)^{-p/2} \begin{vmatrix}\dfrac{g}{(g + 1)\phi}(X^TX)^{-1}\end{vmatrix}^{-1/2}  
\\
exp(-\dfrac{(\beta - \dfrac{g}{g + 1}\hat{\beta}_{OLS})^T (\dfrac{g}{g + 1}\sigma^2(X^TX)^{-1})^{-1} (\beta - \dfrac{g}{g + 1}\hat{\beta}_{OLS})}{2})
\\
...
$$ \propto \int \phi^{(p+\nu_{n})/2 -1} exp(-\dfrac{(\beta - \dfrac{g}{g + 1}\hat{\beta}_{OLS})^T (\dfrac{g + 1}{g }(X^TX))  (\beta - \dfrac{g}{g + 1}\hat{\beta}_{OLS}) + \nu_{n}\hat{\sigma^2}}{2}\phi) d\phi $$

This is the kernel of a gamma and we know that it is equal to the reciprocal of its proportional constant:

$$  = \Gamma(\dfrac{p + \nu_{n}}{2}) \Big[\dfrac{(\beta - \dfrac{g}{g + 1}\hat{\beta}_{OLS})^T (\dfrac{g + 1}{g }(X^TX))  (\beta - \dfrac{g}{g + 1}\hat{\beta}_{OLS}) + \nu_{n}\hat{\sigma^2}}{2}\Big]^{-(p + \nu_{n})/2} $$
$$ \propto \Big[(\beta - \dfrac{g}{g + 1}\hat{\beta}_{OLS})^T (\dfrac{g + 1}{g }(X^TX))  (\beta - \dfrac{g}{g + 1}\hat{\beta}_{OLS}) + \nu_{n}\hat{\sigma^2}\Big]^{-(p + \nu_{n})/2} $$

$$ \propto \Big[1 + \dfrac{1}{\nu_{n}}\dfrac{(\beta - \dfrac{g}{g + 1}\hat{\beta}_{OLS})^T (\dfrac{g + 1}{g }(X^TX))  (\beta - \dfrac{g}{g + 1}\hat{\beta}_{OLS})}{\hat{\sigma^2}}\Big]^{-(p+\nu_{n})/2} $$
...
$$ = \int (2\pi)^{-p/2} \begin{vmatrix}x_i\dfrac{g}{(g + 1)\phi}(X^TX)^{-1}x_i^t\end{vmatrix}^{-1/2}  
\\
exp(-\dfrac{(\beta - x_i\dfrac{g}{g + 1}\hat{\beta}_{OLS})^T (x_i\dfrac{g}{g + 1}\sigma^2(X^TX)^{-1}x_i^t)^{-1} (\beta - x_i\dfrac{g}{g + 1}\hat{\beta}_{OLS})}{2})
\\
\dfrac{(\dfrac{\nu_{n}\hat{\sigma^2}}{2})}{\Gamma(\nu_{n}/2)}^{\nu_{n}/2} \phi^{\nu_{n}/2 - 1} exp(-\dfrac{\nu_{n}\hat{\sigma^2}}{2}) d\phi $$

$$ \propto \int \phi^{p/2} \phi^{\nu_{n}/2 -1} exp(-\dfrac{(\beta - x_i\dfrac{g}{g + 1}\hat{\beta}_{OLS})^T (x_i^{-1}\dfrac{g + 1}{g }\phi(X^TX)(x_i^t)^{-1}) (\beta - x_i \dfrac{g}{g + 1}\hat{\beta}_{OLS})}{2}\phi) 
\\
exp(-\dfrac{\nu_{n}\hat{\sigma^2}}{2}) d\phi $$

$$ \propto \int \phi^{(p+\nu_{n})/2 -1} exp(-\dfrac{(\beta - x_i \dfrac{g}{g + 1}\hat{\beta}_{OLS})^T (x_i^{-1}\dfrac{g + 1}{g }(X^TX)(x_i^t)^{-1})  (\beta - x_i \dfrac{g}{g + 1}\hat{\beta}_{OLS}) + \nu_{n}\hat{\sigma^2}}{2}\phi) d\phi $$
...
$$  = \Gamma(\dfrac{p + \nu_{n}}{2}) \Big[\dfrac{(\beta - x_i \dfrac{g}{g + 1}\hat{\beta}_{OLS})^T (x_i^{-1}\dfrac{g + 1}{g }(X^TX)(x_i^t)^{-1})  (\beta - x_i \dfrac{g}{g + 1}\hat{\beta}_{OLS}) + \nu_{n}\hat{\sigma^2}}{2}\Big]^{-(p + \nu_{n})/2} $$
$$ \propto \Big[\dfrac{(\beta - x_i \dfrac{g}{g + 1}\hat{\beta}_{OLS})^T (x_i^{-1}\dfrac{g + 1}{g }(X^TX)(x_i^t)^{-1})  (\beta - x_i \dfrac{g}{g + 1}\hat{\beta}_{OLS}) + \nu_{n}\hat{\sigma^2}}{2}\Big]^{-(p + \nu_{n})/2} $$

$$ \propto \Big[1 + \dfrac{1}{\nu_{n}}\dfrac{(\beta - x_i\dfrac{g}{g + 1}\hat{\beta}_{OLS})^T (x_i^{-1}\dfrac{g + 1}{g}(X^TX)(x_i^t)^{-1})  (\beta - x_i\dfrac{g}{g + 1}\hat{\beta}_{OLS})}{\hat{\sigma^2}}\Big]^{-(p+\nu_{n})/2} $$

c)

$$ p(Y^{test}| Y, g) = \int p(Y^{test} | Y, g, \phi) p(\phi | Y, g) d\phi $$

$$ = \int (2\pi)^{-p/2} \begin{vmatrix}\dfrac{1}{\phi} ( \dfrac{1}{n} + x_i\dfrac{g}{g + 1}(X^TX)^{-1}x_i^t + I_n)\end{vmatrix}^{-1/2}  
\\
exp(-\dfrac{(Y^{test} - \bar{Y} + x_i \dfrac{g}{1 + g}\hat{\beta}_{OLS})^T (\dfrac{1}{\phi} ( \dfrac{1}{n} + x_i\dfrac{g}{g + 1}(X^TX)^{-1}x_i^t + I_n))^{-1} (Y^{test} - \bar{Y} + x_i \dfrac{g}{1 + g}\hat{\beta}_{OLS})}{2}) \dfrac{(\dfrac{\nu_{n}\hat{\sigma^2}}{2})}{\Gamma(\nu_{n}/2)}^{\nu_{n}/2} \phi^{\nu_{n}/2 - 1} 
\\
exp(-\dfrac{\nu_{n}\hat{\sigma^2}}{2}) d\phi $$

$$ \propto \int \phi^{p/2} \phi^{\nu_{n}/2 -1} 
\\
exp(-\dfrac{(Y^{test} - \bar{Y} + x_i \dfrac{g}{1 + g}\hat{\beta}_{OLS})^T  ( \dfrac{1}{n} + x_i\dfrac{g}{g + 1}(X^TX)^{-1}x_i^t + I_n)^{-1} (Y^{test} - \bar{Y} + x_i \dfrac{g}{1 + g}\hat{\beta}_{OLS})}{2}\phi) 
\\
exp(-\dfrac{\nu_{n}\hat{\sigma^2}}{2}) d\phi $$

$$ \propto \int \phi^{(p+\nu_{n})/2 -1} 
\\
exp(-\dfrac{(Y^{test} - \bar{Y} + x_i \dfrac{g}{1 + g}\hat{\beta}_{OLS})^T  ( \dfrac{1}{n} + x_i\dfrac{g}{g + 1}(X^TX)^{-1}x_i^t + I_n)^{-1} (Y^{test} - \bar{Y} + x_i \dfrac{g}{1 + g}\hat{\beta}_{OLS}) + \nu_{n}\hat{\sigma^2} }{2}\phi) d\phi $$

As in the previous two derivations, we know this is equal to the reciprocal of the proportional constant of the gamma:

$$  = \Gamma(\dfrac{p + \nu_{n}}{2})
\\
\Big[\dfrac{(Y^{test} - \bar{Y} + x_i \dfrac{g}{1 + g}\hat{\beta}_{OLS})^T  ( \dfrac{1}{n} + x_i\dfrac{g}{g + 1}(X^TX)^{-1}x_i^t + I_n)^{-1} (Y^{test} - \bar{Y} + x_i \dfrac{g}{1 + g}\hat{\beta}_{OLS}) + \nu_{n}\hat{\sigma^2} }{2}\Big]^{-(p + \nu_{n})/2} $$

$$  \propto \Big[\dfrac{(Y^{test} - \bar{Y} + x_i \dfrac{g}{1 + g}\hat{\beta}_{OLS})^T  ( \dfrac{1}{n} + x_i\dfrac{g}{g + 1}(X^TX)^{-1}x_i^t + I_n)^{-1} (Y^{test} - \bar{Y} + x_i \dfrac{g}{1 + g}\hat{\beta}_{OLS}) + \nu_{n}\hat{\sigma^2}}{2}\Big]^{-(p + \nu_{n})/2} $$

$$  \propto \Big[1 + \dfrac{1}{\nu_{n}} \dfrac{(Y^{test} - \bar{Y} + x_i \dfrac{g}{1 + g}\hat{\beta}_{OLS})^T  ( \dfrac{1}{n} + x_i\dfrac{g}{g + 1}(X^TX)^{-1}x_i^t + I_n)^{-1} (Y^{test} - \bar{Y} + x_i \dfrac{g}{1 + g}\hat{\beta}_{OLS})}{\hat{\sigma^2}}\Big]^{-(p + \nu_{n})/2} $$
Again, this is proportional to a student-T with $\nu_{n}$ degrees of freedom.
...
It is easy to see that this is the kernel of a Gamma distribution with $a = \dfrac{n + 1}{2}$ and $b = \dfrac{n + (\beta^T\phi(X^T X)\beta)}{2}$ . For this expression to be equal to 1, the proportional constant would need to be $\dfrac{(\dfrac{n + (\beta^T\phi(X^TX)\beta)}{2})^{(n+1)/2}}{\Gamma(\dfrac{n + 1}{2})}$. It can be shown that the reciprocal of this proportional constant is what the above proportion is equal to. In mathematical terms:
