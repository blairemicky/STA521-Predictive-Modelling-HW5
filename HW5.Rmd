...
$$ = \dfrac{(2\pi)^{-p/2} \dfrac{n}{2}^{1/2} \phi^{n/2 - 1}} {\Gamma(\dfrac{1}{2})} \begin{vmatrix} (X^TX)^{-1} \end{vmatrix}^{-1/2} \int \tau^{(n + 1)/2 - 1} exp(-\dfrac{n(\beta^T\phi(X^TX)\beta)}{2} \tau) d\tau $$

It is easy to see that the kernel is that of a Gamma distribution with $a = \dfrac{n + 1}{2}$ and $b = \dfrac{n(\beta^T\phi(X^TX)\beta)}{2} $ . However, the proportional constant is not the same as a Gamma distribution and so this can not, as of yet, integrate to 1. For this expression to be equal to 1, the proportional constant would need to be $ \dfrac{(\dfrac{n(\beta^T\phi(X^TX)\beta)}{2})^{(n+1)/2}}{\Gamma(\dfrac{n + 1}{2})} $. 

We can figure out what the expression above is equal to though. By getting rid of most of the proportional constant above (except for the $\pi$), thinking of what we would need to multiply both sides by for it to be equal to 1, and then taking the reciprocal of that quantity, we find what our Gamma kernel and proportional constant is now equal to. This reciprocal is shown below.

$$ \propto \dfrac{\Gamma(\dfrac{n + 1}{2})}{\pi\Big[(\dfrac{n(\beta^T\phi(X^TX)\beta)}{2})^{(n+1)/2}\Big]} \propto \dfrac{1}{\pi\Big[(\dfrac{\beta^T\beta}{(X^TX/n)^{-1}/\phi})^{(n+1)/2}\Big]}  $$
#COULDN'T FIGURE OUT HOW TO MASSAGE IT INTO CAUCHY FORM; STILL IN PROGRESS - Tom
...
for(i in 1:100){
  rm(df)
  load(fname[i])
  df.bas = bas.lm(Y ~.-mu, data=df[1:50,],
  ...
  pred.df = predict(df.bas, df[51:75,], estimator="HPM")
